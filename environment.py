...
class DataCenterOpsEnv:
    ...
    def get_replay(self) -> ReplayRecord:
        ...
        result = EpisodeResult(
            ...
            score=safe_score(len(self.resolved_incidents) / max(1, total)),
            ...
            coordination_score=self.coordination_score,
            cascade_count=self.cascade_count,
            evidence_efficiency=safe_score(len(self.evidence_gathered) / max(1, self.step_number)),
            communication_efficiency=safe_score(len([m for m in self.message_history if m.message_type != "coordination"]) / max(1, len(self.message_history))),
